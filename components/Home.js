...
function Home() {
  ...
  const [moviesEffect, setMoviesEffect] = useState([]);
  ...
  const movies = moviesEffect.map((data, i) => {
    const isLiked = likedMovies.some((movie) => movie === data.title);
    const newLink = "https://image.tmdb.org/t/p/w500"
    return (
      <Movie
        key={i}
        ...
        poster={newLink+data.poster_path}
